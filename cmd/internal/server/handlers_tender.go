...
//
// Хэндлер выполняет параллельную загрузку данных для повышения производительности:
//   - Основная информация о тендере
//   - Список лотов тендера с пагинацией
//   - Победители по каждому лоту (опционально, graceful degradation)
//
// HTTP метод: GET
// Путь: /api/v1/tenders/:id
//
// Параметры пути:
//   - id (int64): Идентификатор тендера
//
// Query параметры:
//   - limit (int): Количество лотов на странице (по умолчанию 100, диапазон: 1-100)
//   - offset (int): Смещение для пагинации (по умолчанию 0, минимум: 0)
//
// Ответы:
//   - 200: Успешное получение данных (TenderDetailsResponse)
//   - 400: Неверный формат ID или параметров запроса
//   - 404: Тендер не найден
//   - 500: Внутренняя ошибка сервера
//
// Примечания:
//   - Если загрузка победителей не удалась для какого-либо лота, это логируется как warning,
//     но не приводит к ошибке всего запроса (graceful degradation)
//   - Все операции загрузки выполняются параллельно через errgroup для оптимизации времени отклика
