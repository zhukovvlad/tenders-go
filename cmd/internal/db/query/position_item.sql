...
-- 
-- ПРИМЕЧАНИЕ: Запрос имеет защитный LIMIT 10000 для предотвращения чрезмерного
-- потребления памяти. Этого достаточно для 99.9% реальных предложений (обычно 500-2000 позиций).
-- Рекомендуется использовать виртуализацию (react-window) на фронтенде для больших списков.
SELECT
    pi.id,
    pi.proposal_id,
...
ORDER BY
    ...
    pi.id ASC
LIMIT 10000; -- Защитный лимит для предотвращения OOM на экстремальных объемах
