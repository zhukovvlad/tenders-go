...
--
-- === Компонент 0: Исправление `position_items` (CRITICAL) ===
-- * Проблема: В `000001_init_schema` поле `catalog_position_id` было `NOT NULL`.
--   Это ломает нашу новую асинхронную логику.
-- * Решение: Мы делаем это поле `NULLABLE`. Теперь Go-сервер может при
--   "Cache Miss" (промахе кэша) ставить `NULL`, а Python-воркер в фоне
--   будет находить эти `NULL` и заполнять их.
--
...
-- --- ШАГ 0: (КРИТИЧЕСКИ ВАЖНО) Исправление `position_items` ---
-- Делаем `catalog_position_id` NULLABLE, чтобы Go-сервер мог
-- записывать NULL при "Cache Miss", а Python-воркер их "догонял".
ALTER TABLE "position_items"
ALTER COLUMN "catalog_position_id" DROP NOT NULL;

COMMENT ON COLUMN "position_items"."catalog_position_id" IS 'Ссылка на каталог (NULLABLE). NULL означает, что Python-воркер еще не обработал эту позицию.';
