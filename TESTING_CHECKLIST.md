...
- [ ] Тест GroupPositions — parent_id конфликт без force → ConflictError с деталями (position_id, title, current_parent_id, siblings_count)
- [ ] Тест GroupPositions — parent_id конфликт с force=true → успешная перезапись
...
#### Ungroup Position (UngroupPosition) — unit-тесты

**Новые тесты:**
- [ ] Тест UngroupPosition — успешное исключение из группы (UngroupPosition + DeleteGroupedMergesForPosition в транзакции)
- [ ] Тест UngroupPosition — позиция не найдена или parent_id IS NULL → sql.ErrNoRows → ValidationError
- [ ] Тест UngroupPosition — positionID <= 0 (ValidationError)
- [ ] Тест UngroupPosition — пустой executedBy (ValidationError)
- [ ] Тест UngroupPosition — executedBy из пробелов (ValidationError после TrimSpace)
- [ ] Тест UngroupPosition — ошибка БД UngroupPosition (wrapped error, транзакция откатывается)
- [ ] Тест UngroupPosition — ошибка БД DeleteGroupedMergesForPosition (wrapped error, транзакция откатывается)
- [ ] Тест UngroupPosition — DeleteGroupedMergesForPosition вызывается точно с positionID (не с другим id)
- [ ] Тест UngroupPosition — ошибка ExecTx (tx begin failed)

#### List Catalog Groups (ListGroups, ListGroupChildren) — unit-тесты

**Новые тесты:**
- [ ] Тест ListGroups — успешное получение списка групп (Groups, Total)
- [ ] Тест ListGroups — пустой каталог (Groups=[], Total=0)
- [ ] Тест ListGroups — Total из CountGroups, а не len(Groups)
- [ ] Тест ListGroups — корректная пагинация (limit, offset передаются в ListGroupsParams)
- [ ] Тест ListGroups — Description=nil когда sql.NullString{Valid:false}
- [ ] Тест ListGroups — Description=*string когда sql.NullString{Valid:true}
- [ ] Тест ListGroups — ChildrenCount в GroupSummary совпадает с returned int32
- [ ] Тест ListGroups — limit <= 0 (ValidationError)
- [ ] Тест ListGroups — offset < 0 (ValidationError)
- [ ] Тест ListGroups — ошибка БД CountGroups (wrapped error)
- [ ] Тест ListGroups — ошибка БД ListGroups (wrapped error)
- [ ] Тест ListGroupChildren — успешное получение дочерних позиций
- [ ] Тест ListGroupChildren — родитель без детей (Children=[], ParentID совпадает с аргументом)
- [ ] Тест ListGroupChildren — Description=nil / Description=*string (оба кейса)
- [ ] Тест ListGroupChildren — Kind и Status корректно маппятся в CatalogPositionSummary
...
- [ ] Тест GroupBatchPositions с NewParentTitle — response содержит MergeIDs, ParentID, PositionIDs (sorted), Status="GROUPED"
- [ ] Тест GroupBatchPositions с ParentID — успешная группировка (GroupMergeBatch + GetCatalogPositionByID + N× SetPositionParent)
- [ ] Тест GroupBatchPositions — частичный batch: часть merge_ids не PENDING/APPROVED (ValidationError с failedIDs)
...
- [ ] Тест GroupBatchPositions — parent_id конфликт с force=true → успех
- [ ] Тест GroupBatchPositions — идемпотентность: позиция уже под целевым parent → не считается конфликтом
- [ ] Тест GroupBatchPositions — дедупликация position_ids из merge-записей (позиция в нескольких merge)
- [ ] Тест GroupBatchPositions — детерминированный порядок: positionIDs отсортированы
- [ ] Тест GroupBatchPositions — ошибка БД GroupMergeBatch (wrapped DB error)
- [ ] Тест GroupBatchPositions — ошибка SetPositionParent (ValidationError: позиция deprecated/merged)
- [ ] Тест GroupBatchPositions — ошибка БД SetPositionParent (wrapped DB error)
...
**GroupMergeBatch:**
- [ ] Тест `GroupMergeBatch` — успешный bulk PENDING/APPROVED → GROUPED
- [ ] Тест `GroupMergeBatch` — частичный: часть merge_ids в неверном статусе → len(returned) < len(ids)
- [ ] Тест `GroupMergeBatch` — все merge_ids в неверном статусе → пустой результат
- [ ] Тест `GroupMergeBatch` — несуществующие ID → не возвращаются

**CountPositionsByParentID:**
- [ ] Тест `CountPositionsByParentID` — родитель с N детьми → N
- [ ] Тест `CountPositionsByParentID` — родитель без детей → 0
- [ ] Тест `CountPositionsByParentID` — несуществующий parent_id → 0
...
**ListGroups / CountGroups / ListGroupChildren (2026-03-11):**
- [ ] Тест `CountGroups` — возвращает 0 когда нет GROUP_TITLE позиций
- [ ] Тест `CountGroups` — игнорирует POSITION/HEADER/TRASH (считает только GROUP_TITLE)
- [ ] Тест `ListGroups` — возвращает только GROUP_TITLE позиции
- [ ] Тест `ListGroups` — children_count корректен (коррелирующий подзапрос по parent_id)
- [ ] Тест `ListGroups` — children_count = 0 для группы без дочерних позиций
- [ ] Тест `ListGroups` — LIMIT/OFFSET: LIMIT=1 возвращает одну группу
- [ ] Тест `ListGroups` — LIMIT/OFFSET: OFFSET пропускает группы
- [ ] Тест `ListGroups` — порядок: ORDER BY standard_job_title ASC
- [ ] Тест `ListGroupChildren` — возвращает только позиции с parent_id = $1
- [ ] Тест `ListGroupChildren` — не возвращает позиции с другим parent_id
- [ ] Тест `ListGroupChildren` — пустой результат для несуществующего parent_id
- [ ] Тест `ListGroupChildren` — порядок: ORDER BY standard_job_title ASC

**UngroupPosition / DeleteGroupedMergesForPosition (2026-03-11):**
- [ ] Тест `UngroupPosition` — сбрасывает parent_id в NULL, status → pending_indexing, updated_at обновлён
- [ ] Тест `UngroupPosition` — parent_id IS NULL (позиция не в группе) → sql.ErrNoRows
- [ ] Тест `UngroupPosition` — id не существует → sql.ErrNoRows
- [ ] Тест `DeleteGroupedMergesForPosition` — удаляет GROUPED-записи где main_position_id = $1
- [ ] Тест `DeleteGroupedMergesForPosition` — удаляет GROUPED-записи где duplicate_position_id = $1
- [ ] Тест `DeleteGroupedMergesForPosition` — не трогает PENDING/APPROVED/REJECTED/EXECUTED для той же позиции
- [ ] Тест `DeleteGroupedMergesForPosition` — не трогает GROUPED-записи других позиций
- [ ] Тест `DeleteGroupedMergesForPosition` — нет строк (no-op, без ошибки)
- [ ] Тест интеграционный: UngroupPosition + DeleteGroupedMergesForPosition в транзакции — атомарность (ошибка второго шага откатывает первый)
...
- [ ] Тест `UpsertSuggestedMerge` — позиция в статусе GROUPED → повторный upsert НЕ сбрасывает в PENDING
...
- [ ] Тесты GroupPositionsHandler (`POST /api/v1/admin/merges/:id/group`)
  ...
  - [ ] Конфликт parent_id (с force=true) → 200 OK
  - [ ] Ошибка БД → 500 (generic: `{"error": "internal_server_error", "message": "internal server error"}`, без утечки деталей)
  - [ ] Проверка требования роли admin
  - [ ] Проверка user_id из JWT передаётся как executedBy
- [ ] Тесты GroupBatchPositionsHandler (`POST /api/v1/admin/merges/group-batch`)
  - [ ] Успешная группировка с NewParentTitle → 200 + GroupBatchPositionsResponse
  - [ ] Успешная группировка с ParentID → 200 + GroupBatchPositionsResponse
  - [ ] Пустое тело запроса → 400
  - [ ] Невалидный JSON body (неизвестные поля) → 400
  - [ ] Пустой merge_ids → 400
...
- [ ] Тесты ListGroupsHandler (`GET /api/v1/admin/catalog/groups`)
  - [ ] Успешный запрос → 200 + `{"groups": [...], "total": N}`
  - [ ] Пустой каталог → 200 + `{"groups": [], "total": 0}`
  - [ ] limit и offset передаются корректно (дефолты: limit=50, offset=0)
  - [ ] Невалидный limit (строка, отрицательный, ноль) → 400
  - [ ] Невалидный offset (строка, отрицательный) → 400
  - [ ] Ошибка сервиса → 500
  - [ ] Пользователь не admin → 403
  - [ ] Unauthorized (нет JWT) → 401
- [ ] Тесты ListGroupChildrenHandler (`GET /api/v1/admin/catalog/groups/:id/children`)
  - [ ] Успешный запрос → 200 + `{"children": [...], "parent_id": <id>}`
  - [ ] Родитель без детей → 200 + `{"children": [], "parent_id": <id>}`
  - [ ] Невалидный ID (строка) → 400
  - [ ] ID <= 0 → 400
  - [ ] Ошибка сервиса → 500
  - [ ] Пользователь не admin → 403
  - [ ] Unauthorized (нет JWT) → 401
- [ ] Тесты UngroupPositionHandler (`POST /api/v1/admin/catalog/positions/:id/ungroup`)
  - [ ] Успешное исключение → 200 + `{"status": "ungrouped", "position_id": <id>}`
  - [ ] Невалидный ID (строка) → 400
  - [ ] ID <= 0 → 400
  - [ ] Позиция не в группе (ValidationError от сервиса) → 400
  - [ ] Ошибка сервиса → 500
  - [ ] Пользователь не admin → 403
  - [ ] Unauthorized (нет JWT) → 401
