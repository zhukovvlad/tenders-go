...
2. Проверяет существование merge через `GetSuggestedMergeByID`
3. Проверяет, что текущий статус — `PENDING` (нельзя отклонить уже исполнённый или отклонённый)
4. Вызывает `UpdateMergeStatus` с `status=REJECTED`, `resolved_by=userID`
