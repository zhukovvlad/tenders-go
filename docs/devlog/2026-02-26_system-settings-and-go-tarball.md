...
**DOWN-миграция:**
- `DROP TABLE ... CASCADE` (удаляет зависимые триггеры) → `DROP FUNCTION`
...
### Принятые замечания — раунд 2 (2)
4. **UP: `updated_by` NOT NULL DEFAULT 'system'** — аудитная колонка без NOT NULL не гарантирует
   фиксацию автора изменения. `DEFAULT 'system'` покрывает seed и программные обновления.
5. **Devlog: DOWN description** — обновлено описание DOWN-миграции: `DROP TABLE CASCADE` → `DROP FUNCTION`
   (вместо устаревшего "DROP TRIGGER + DROP FUNCTION + DROP TABLE").

### Добавлено в Makefile
- `make migratedown1` — откат ровно одной миграции (`down 1`), без интерактивного подтверждения.

Применение: rollback 000005 → правка файлов → `make sqlc` → reapply 000005.
...
- `Makefile` (добавлен `migratedown1`)
- `TESTING_CHECKLIST.md` (обновлён)

### Локальные изменения окружения (не включены в PR)

- `~/.bashrc` — добавлен `export PATH=/usr/local/go/bin:$HOME/go/bin:$PATH` (переход с snap на tarball)
